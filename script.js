...
function generatePassword () {
    ...
    var verifyLeastNum = Number(window.prompt("Enter the minimum number of the password length. (It can't be lower than 8 characters.)","8"));
        if ( !verifyLeastNum) {
            console.log("Error: User has canceled the selection. Please check if user entered valid numbers.");
            window.alert("Error: Generating password has been canceled. Please check if you entered valid numbers.")
            return;
            ...
            window.alert("You can not select minimum number more than 128 characters. Please try it again.")
            console.log("User has selected invalid number.")
            return;
        }    else {
    ...
    var verifyMaxNum = Number(window.prompt("Enter the maximum number of the password length. (It can't exceed 128)", `${verifyLeastNum}`));
        if ( !verifyMaxNum) {
            ...
            window.alert("Maximum number is lower than minimum number. Please try it again.");
            console.log("User has selected invalid number.");
            return;
        } else if (verifyMaxNum > "128") {
            window.alert("Maximum number can not be more than 128 characters. Please try it again.");
            console.log("User has selected invalid number.");
            return;
        } else {
    ...
    var verifyLowercase = window.prompt("Do you want to include lowercase?","Yes / No");
    if (!verifyLowercase) {
        console.log("User has canceled the selection.");
        window.alert("Generating password has been canceled.")
            return;
            ...
            console.log("User has included the lowercase letters into the final array.");
        } 
        else if(verifyLowercase.toLowerCase() === `no`) {
            console.log("User has not included the lowercase letters into the final array.");
    } else {
    ...
    var verifyUppercase = window.prompt("Do you want to include uppercase?","Yes / No");
        if (!verifyUppercase) {
            console.log("User has canceled the selection.");
            window.alert("Generating password has been canceled.")
            return;
            ...
            console.log("User has included the uppercase letters into the final array.");
        } 
        else if(verifyUppercase.toLowerCase() === `no`) {
            console.log("User has not included the uppercase letters into the final array.");
    } else {
        window.alert(`This is invalid answer, please try again`);
        console.log("User has selected invalid answer.");
        return;
    ...
    var verifyNumeric = window.prompt("Do you want to include numeric characters?","Yes / No");
    if (!verifyNumeric) {
        console.log("User has canceled the selection.");
        window.alert("Generating password has been canceled.");
        return;
        ...
        console.log("User has included the numeric characters into the final array.");
    } 
    else if(verifyNumeric.toLowerCase() === `no`) {
    console.log("User has not included the numeric characters into the final array.");
} else {
    window.alert(`This is invalid answer, please try again`);
    console.log("User has selected invalid answer.");
    return;
    ...
    var verifySpecial = window.prompt("Do you want to include special characters?","Yes / No");
    if (!verifySpecial) {
        console.log("User has canceled the selection.");
        window.alert("Generating password has been canceled.");
        return;
...
if (finalArray.length < 1) {
    window.alert("Please select at least one type of characters to include it in password.")
    return;
}


// Randomize the password by creating length of password and randomize the index of final array
    var passwordLength = Math.floor(Math.random() * (verifyMaxNum - verifyLeastNum)) + verifyLeastNum;
    console.log(`The length of the password has ${passwordLength} characters.`);
    
    let resultPassword = "";
    for (let i = 0; i < passwordLength; i++) {
        resultPassword += finalArray[
            Math.floor(Math.random() * finalArray.length)]
    }
